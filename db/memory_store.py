...
def _search_and_filter(
    ...
    agent_id is NOT used for filtering — see the module comment above:
    this SDK version doesn't persist it, confirmed by prior investigation.
    """
    try:
        ...
        filtered = [
            r for r in result
            if isinstance(r, dict) and _matches_tenant(r.get("memory", ""), tenant_id, session_id)
        ]
...
# _save_to_mem0() removed — confirmed dead code (zero import sites anywhere
# in the codebase, checked exhaustively). It duplicated add_conversation_turn's
# logic and never ran; its stray `return _mem0_client` (fixed in an earlier
# pass, before this removal) was almost certainly a copy-paste artifact from
# _get_client(), which is where that line actually belonged.
...
async def save_workflow_snapshot(
    ...
    """Delegates saving of workflow snapshots to MemoryManager."""
    try:
        from ai.memory_manager import MemoryManager
        mm = MemoryManager(tenant_id, session_id)
        await mm.save_workflow_snapshot(
            state=state,
            product=product,
            quantity=quantity,
