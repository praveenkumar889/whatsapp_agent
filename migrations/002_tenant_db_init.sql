-- ══════════════════════════════════════════════════════════════════════════════
-- MIGRATION 002 — Tenant DB Initialization & Complete Prompt Migration
-- Run this ONE file in the Supabase SQL Editor of your TENANT DB
-- (https://fxzjcchyjqxvxuvoyron.supabase.co)
-- ══════════════════════════════════════════════════════════════════════════════
...
CREATE TABLE IF NOT EXISTS tenants (
    ...
    graphrag_api_url TEXT,
    products_api_url TEXT,
    access_token TEXT,
    valid_intents TEXT,
    intent_system_prompt TEXT,
    greeting_system_prompt TEXT,
    entity_system_prompt TEXT,
    graphrag_system_prompt TEXT,
    escalation_prompt TEXT,
    negotiation_prompt TEXT,
    unknown_prompt TEXT,
    invoice_inquiry_prompt TEXT,
    invoice_confirm_prompt TEXT,
    invoice_order_confirm_prompt TEXT,
    invoice_confirmation_prompt TEXT,
    neg_is_request_prompt TEXT,
    neg_extract_qty_prompt TEXT,
    neg_detect_counter_prompt TEXT,
    neg_more_discount_prompt TEXT,
    neg_detect_accept_prompt TEXT,
    neg_detect_qty_change_prompt TEXT,
    neg_no_discount_prompt TEXT,
    neg_first_offer_prompt TEXT,
    neg_counter_offer_prompt TEXT,
    neg_final_price_prompt TEXT,
    fast_confirm_prompt TEXT,
    product_summary_recommendation_prompt TEXT,
    acceptance_keywords TEXT,
    acceptance_exact_words TEXT,
    parse_global_offer_tiers_prompt TEXT,
    invoice_inquiry_check_prompt TEXT,
    order_confirmation_reply_check_prompt TEXT,
    generate_invoice_cta_prompt TEXT,
    invoice_confirmation_request_check_prompt TEXT,
    fast_order_confirm_check_prompt TEXT,
    category_matcher_prompt TEXT,
    pf_data_extraction_prompt TEXT,
    pf_history_resolver_prompt TEXT,
    pf_offer_inquiry_check_prompt TEXT,
    pf_neg_product_change_check_prompt TEXT,
    pf_vague_reference_check_prompt TEXT,
    pf_vague_reference_rewriter_prompt TEXT,
    pf_offer_inquiry_check_l2_prompt TEXT,
    pf_named_product_extractor_prompt TEXT,
    pf_offers_formatter_prompt TEXT,
    pf_vague_pronoun_resolver_l2_prompt TEXT,
    pf_comparison_prompt TEXT,
    pf_image_installation_intent_prompt TEXT,
    pf_main_followup_prompt TEXT,
    pf_new_search_followup_classifier_prompt TEXT
);

-- Ensure new prompt columns exist if table was already created:
...
-- ── Insert or Update Inventaa Tenant Profile with ALL Prompts ──
INSERT INTO tenants (tenant_id, phone_number_id, waba_id, biz_name, region, timezone, language, support_email, website, city, upi_id, account_name, tagline, gst_rate, support_phone, graphrag_api_url, products_api_url, access_token, valid_intents, intent_system_prompt, greeting_system_prompt, entity_system_prompt, graphrag_system_prompt, escalation_prompt, negotiation_prompt, unknown_prompt, invoice_inquiry_prompt, invoice_confirm_prompt, invoice_order_confirm_prompt, invoice_confirmation_prompt, neg_is_request_prompt, neg_extract_qty_prompt, neg_detect_counter_prompt, neg_more_discount_prompt, neg_detect_accept_prompt, neg_detect_qty_change_prompt, neg_no_discount_prompt, neg_first_offer_prompt, neg_counter_offer_prompt, neg_final_price_prompt, fast_confirm_prompt, product_summary_recommendation_prompt, acceptance_keywords, acceptance_exact_words, parse_global_offer_tiers_prompt, invoice_inquiry_check_prompt, order_confirmation_reply_check_prompt, generate_invoice_cta_prompt, invoice_confirmation_request_check_prompt, fast_order_confirm_check_prompt, category_matcher_prompt, pf_data_extraction_prompt, pf_history_resolver_prompt, pf_offer_inquiry_check_prompt, pf_neg_product_change_check_prompt, pf_vague_reference_check_prompt, pf_vague_reference_rewriter_prompt, pf_offer_inquiry_check_l2_prompt, pf_named_product_extractor_prompt, pf_offers_formatter_prompt, pf_vague_pronoun_resolver_l2_prompt, pf_comparison_prompt, pf_image_installation_intent_prompt, pf_main_followup_prompt, pf_new_search_followup_classifier_prompt)
VALUES ('tenant_inventaa_led_001', '1124766240726230', '206253068645466', 'Inventaa LED Lights', 'india', 'Asia/Kolkata', 'en', 'support@inventaa.in', 'inventaa.in', 'Chennai, Tamil Nadu', 'inventaa@upi', 'Inventaa LED Innovation Pvt Ltd', 'LED Lighting Solutions | Made in India', 18, '+91 72990 39181', NULL, NULL, NULL, NULL, 'You are an intent classification AI for Inventaa LED Lights — an Indian LED lighting manufacturer based in Chennai.
...
Output ONLY: {"intent": "WORKFLOW_ACTION", "confidence_score": 0.97}', 'You are a friendly WhatsApp assistant for Inventaa LED Lights — LED Lighting Solutions | Made in India, Chennai, Tamil Nadu.
...
{"type": "GENERAL", "reply": "full reply text here"}', 'You are an entity extraction AI for Inventaa LED Lights — Indian LED lighting manufacturer.
...
Examples:
  ...
  → [{"product_name": null, "quantity_value": 2, "quantity_unit": "units"}]', 'You are an expert LED lighting sales assistant for Inventaa LED Lights. Answer the customer''s query using the provided catalog search results. Be helpful, concise, and highlight product specifications like wattage, warranty, and IP rating when relevant.', 'You are a warm, empathetic WhatsApp assistant for Inventaa LED Lights.
Support: support@inventaa.in | +91 72990 39181 | inventaa.in
...
Reply ONLY with message text — NO JSON, no explanation.', 'You are negotiating price with a customer for Inventaa LED Lights. Be polite, professional, and stand firm on discounts unless the volume justifies a tier discount. Never offer discounts below the minimum margin.', 'You are a friendly WhatsApp assistant for Inventaa LED Lights.
...
Reply ONLY with message text — no JSON, no explanation.', 'Is the customer explicitly asking for their invoice, receipt, bill, or payment document?
...
Reply ONLY "YES" or "NO".', 'Does the assistant reply text indicate that an order has been confirmed, placed, processed, or scheduled?
Examples YES: "Thank you for confirming", "Your order is now being processed", "order confirmed"
Reply ONLY "YES" or "NO".', 'You are a WhatsApp assistant for {biz_name}.
...
Example: Reply "Proceed" or "Confirm" to get your invoice right away! 📄', 'The bot previously asked the customer to reply "Confirm" or "Proceed" to generate their invoice.
...
Reply ONLY "YES" or "NO".', 'Is the customer asking for a price discount, negotiating price, saying it is too high, or asking for any deal/offer/reduction?
...
Reply ONLY "YES" or "NO".', 'Customer is ordering {product_name}.
Extract the quantity (integer number of units) from their message.
Reply ONLY with the integer number, or "NONE" if no quantity found.', 'The customer may be proposing a specific price.
...
Reply ONLY one of:
  UNIT:<number>
  TOTAL:<number>
  NONE', 'Is the customer asking for more/further/additional discount or a better price WITHOUT mentioning a specific price number?
...
Reply ONLY "YES" or "NO".', 'You are detecting whether a customer has ACCEPTED a price offer.
...
Customer message: {message}', 'Customer currently has {current_qty} units of {product_name} in their order.
...
Reply ONLY with the integer or "NONE".', 'You are a friendly sales assistant for {biz_name}.
...
Reply ONLY with message text.', 'You are a friendly sales assistant for {biz_name}.
...
Reply ONLY with message text.', 'You are a warm, professional sales negotiator for {biz_name}.
...
- Reply ONLY with the message text', 'You are a warm but firm sales negotiator for {biz_name}.
...
- Reply ONLY with the message text', 'Bot showed an order summary and asked the customer to confirm. Is the customer confirming to place the order?
...
Reply ONLY "YES" or "NO".', 'You are a helpful WhatsApp sales assistant for {biz_name}.
...
- Reply ONLY with the summary block text — no JSON, no explanation', 'confirm,proceed,yes confirm,go ahead,place order,confirm this,confirm order,place this order,book it,book this,let''s do it,sounds good,works for me,okay confirmed,continue with this,place it,send invoice,generate invoice,checkout,pay now', 'yes,ok,okay,sure,done,fine,alright,great,perfect,deal,accepted', 'Parse the following global offer tier text into a JSON list of lists where each element is [min_quantity, discount_percentage], sorted by min_quantity ascending. For example: [[5, 5], [10, 10], [20, 15]]. Return ONLY valid JSON, no markdown formatting or extra text.', 'You are an intent classification assistant for an e-commerce WhatsApp bot. Determine if the customer''s message is asking for their invoice, bill, receipt, or payment document. Reply EXACTLY "YES" or "NO".', 'Determine if the customer is confirming an order in response to an order confirmation prompt. Look for affirmative words like "yes", "proceed", "confirm", "ok", or "go ahead". Reply EXACTLY "YES" or "NO".', 'Generate a friendly, concise call-to-action message asking the customer to confirm their order so you can automatically generate and send their tax invoice for {biz_name}. Keep it under 2 sentences and tell them to reply "Proceed" or "Confirm".', 'Determine if the user''s reply is confirming that they want to generate their invoice in response to the assistant asking them to confirm. Look for affirmative expressions like "Proceed", "Confirm", "Yes", "Ok". Reply EXACTLY "YES" or "NO".', 'Determine if the customer''s message is a direct, fast order confirmation (e.g., "confirm order", "place order now", "yes order it"). Reply EXACTLY "YES" or "NO".', 'Match the customer''s lighting query to one or more of the following product categories: {cat_list}. Return a comma-separated list of matching category names. If no category matches, return "NONE".', 'Extract product specifications, quantities, and preferences from the conversation history and current message. Return JSON with keys: "product_name", "quantity", "unit", "color", "wattage". If a field is not specified, set it to null.', 'Analyze the conversation history to resolve which specific lighting product the customer is referring to. Return ONLY the exact product name from the catalog or conversation history.', 'Determine if the customer is inquiring about discounts, bulk pricing, or offers on a product. Reply EXACTLY "YES" or "NO".', 'Determine if the customer is switching to a different product during negotiation instead of negotiating on the current product ({current_product}). Reply EXACTLY "YES" or "NO".', 'Determine if the customer''s message contains vague references or pronouns (like "this one", "that light", "the 12w one", "it") without explicitly naming the product. Reply EXACTLY "YES" or "NO".', 'Rewrite the customer''s message by replacing vague pronouns and references with the actual product name: "{product_name}". Return ONLY the rewritten message.', 'Determine if the customer is specifically asking for pricing tiers, bulk discount percentages, or special offers. Reply EXACTLY "YES" or "NO".', 'Extract the explicit product name mentioned by the customer from their text. Return ONLY the product name, or "NONE" if no specific product is named.', 'Format the available discount tiers and bulk pricing offers into a clean, easy-to-read WhatsApp message with emojis and clear bullet points.', 'Resolve ambiguous pronouns (it, this, that, those) to the most recently discussed product in the conversation history. Return ONLY the resolved product name.', 'Compare the specified lighting products based on their specifications, wattage, price, warranty, and best use cases. Format as a concise, readable comparison for WhatsApp.', 'Determine if the customer is asking for installation photos, product images, diagrams, or visual examples of how the light looks installed. Reply EXACTLY "YES" or "NO".', 'You are an AI sales assistant for Inventaa LED Lights. Answer the customer''s follow-up question using the provided product context: {catalog_data}. Be friendly, helpful, and concise.', 'Classify whether the customer''s message is a follow-up about the currently discussed product OR a brand new search for a completely different product. Reply EXACTLY "FOLLOWUP" or "NEW_SEARCH".')
ON CONFLICT (tenant_id) DO UPDATE SET
phone_number_id = EXCLUDED.phone_number_id, waba_id = EXCLUDED.waba_id, biz_name = EXCLUDED.biz_name, region = EXCLUDED.region, timezone = EXCLUDED.timezone, language = EXCLUDED.language, support_email = EXCLUDED.support_email, website = EXCLUDED.website, city = EXCLUDED.city, upi_id = EXCLUDED.upi_id, account_name = EXCLUDED.account_name, tagline = EXCLUDED.tagline, gst_rate = EXCLUDED.gst_rate, support_phone = EXCLUDED.support_phone, graphrag_api_url = EXCLUDED.graphrag_api_url, products_api_url = EXCLUDED.products_api_url, access_token = EXCLUDED.access_token, valid_intents = EXCLUDED.valid_intents, intent_system_prompt = EXCLUDED.intent_system_prompt, greeting_system_prompt = EXCLUDED.greeting_system_prompt, entity_system_prompt = EXCLUDED.entity_system_prompt, graphrag_system_prompt = EXCLUDED.graphrag_system_prompt, escalation_prompt = EXCLUDED.escalation_prompt, negotiation_prompt = EXCLUDED.negotiation_prompt, unknown_prompt = EXCLUDED.unknown_prompt, invoice_inquiry_prompt = EXCLUDED.invoice_inquiry_prompt, invoice_confirm_prompt = EXCLUDED.invoice_confirm_prompt, invoice_order_confirm_prompt = EXCLUDED.invoice_order_confirm_prompt, invoice_confirmation_prompt = EXCLUDED.invoice_confirmation_prompt, neg_is_request_prompt = EXCLUDED.neg_is_request_prompt, neg_extract_qty_prompt = EXCLUDED.neg_extract_qty_prompt, neg_detect_counter_prompt = EXCLUDED.neg_detect_counter_prompt, neg_more_discount_prompt = EXCLUDED.neg_more_discount_prompt, neg_detect_accept_prompt = EXCLUDED.neg_detect_accept_prompt, neg_detect_qty_change_prompt = EXCLUDED.neg_detect_qty_change_prompt, neg_no_discount_prompt = EXCLUDED.neg_no_discount_prompt, neg_first_offer_prompt = EXCLUDED.neg_first_offer_prompt, neg_counter_offer_prompt = EXCLUDED.neg_counter_offer_prompt, neg_final_price_prompt = EXCLUDED.neg_final_price_prompt, fast_confirm_prompt = EXCLUDED.fast_confirm_prompt, product_summary_recommendation_prompt = EXCLUDED.product_summary_recommendation_prompt, acceptance_keywords = EXCLUDED.acceptance_keywords, acceptance_exact_words = EXCLUDED.acceptance_exact_words, parse_global_offer_tiers_prompt = EXCLUDED.parse_global_offer_tiers_prompt, invoice_inquiry_check_prompt = EXCLUDED.invoice_inquiry_check_prompt, order_confirmation_reply_check_prompt = EXCLUDED.order_confirmation_reply_check_prompt, generate_invoice_cta_prompt = EXCLUDED.generate_invoice_cta_prompt, invoice_confirmation_request_check_prompt = EXCLUDED.invoice_confirmation_request_check_prompt, fast_order_confirm_check_prompt = EXCLUDED.fast_order_confirm_check_prompt, category_matcher_prompt = EXCLUDED.category_matcher_prompt, pf_data_extraction_prompt = EXCLUDED.pf_data_extraction_prompt, pf_history_resolver_prompt = EXCLUDED.pf_history_resolver_prompt, pf_offer_inquiry_check_prompt = EXCLUDED.pf_offer_inquiry_check_prompt, pf_neg_product_change_check_prompt = EXCLUDED.pf_neg_product_change_check_prompt, pf_vague_reference_check_prompt = EXCLUDED.pf_vague_reference_check_prompt, pf_vague_reference_rewriter_prompt = EXCLUDED.pf_vague_reference_rewriter_prompt, pf_offer_inquiry_check_l2_prompt = EXCLUDED.pf_offer_inquiry_check_l2_prompt, pf_named_product_extractor_prompt = EXCLUDED.pf_named_product_extractor_prompt, pf_offers_formatter_prompt = EXCLUDED.pf_offers_formatter_prompt, pf_vague_pronoun_resolver_l2_prompt = EXCLUDED.pf_vague_pronoun_resolver_l2_prompt, pf_comparison_prompt = EXCLUDED.pf_comparison_prompt, pf_image_installation_intent_prompt = EXCLUDED.pf_image_installation_intent_prompt, pf_main_followup_prompt = EXCLUDED.pf_main_followup_prompt, pf_new_search_followup_classifier_prompt = EXCLUDED.pf_new_search_followup_classifier_prompt;